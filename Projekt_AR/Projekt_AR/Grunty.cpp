...
**/
void Grunty::wyswietOpis() {

	cout << "Grunty" << "Gotowosc wyceny: " << zamienBool() << " Adres: " << miastoWyc() << " " << ulicaWyc() << " " << numerWyc() << " Powierzchnia w metrach kwadratowych: " << powWyc() << " Stan gruntu: " << zamienstan() << " Uzbrojenie: " << zamienUzbr() << " " << endl;
}

/**
* Metoda zapisajaca opis obiektu konkretnej klasy do pliku w celu odczytu przez program.
**/
void Grunty::zapiszOpis(fstream& plik) {
	plik << "-Grunty-" << czygotoweWyc() << "-" << miastoWyc() << "-" << ulicaWyc() << "-" << numerWyc() << "-" << powWyc() << "-" << stan_gruntuWyc() << "-" << uzbrojenieWyc() << endl;
}

/**
* Metoda zapisajaca opis obiektu konkretnej klasy do pliku w celu odczytu przez uzytkownika.
**/
void Grunty::zapiszOpis2(ofstream& plik) {

	plik << "Grunt " << "Gotowosc wyceny: " << zamienBool() << " Adres: " << miastoWyc() << " " << ulicaWyc() << " " << numerWyc() << " Powierzchnia w metrach kwadratowych: " << powWyc() << " Stan gruntu: " << zamienstan() << " Uzbrojenie: " << zamienUzbr() << " " << endl;
